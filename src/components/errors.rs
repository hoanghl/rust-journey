...
/// - Incorrect packet id
/// - Incorrect minimum header size
/// - Mismatched packet size
...
impl Display for ParseError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "ParseError{{error_code: {}, packet_id: {:?}, header_size: {:?}, payload_size: {:?}, packet_size: {:?}}}",
            self.error_code, self.packet_id, self.header_size, self.payload_size, self.packet_size
        )
    }
}
...
impl ParseError {
    ...
    pub fn incorrect_packet_id(packet_id: u8) -> ParseError {
    ...
    pub fn mismatched_packet_size(packet_size: usize, payload_size: usize) -> ParseError {
        let mut err = ParseError::create_instance();
...
/// Handle following cases:
/// - Receiver thread creation error
/// - Processor thread creation error
/// - Sender thread creation error
